...
//==============================================================================
    ...
    void JUCE_CALLTYPE NativeMessageBox::showTextEditBox(const String &title,
                                                         const String &message,
                                                         const String &textPlaceholder,
                                                         const String  &prepopulatedText,
                                                         std::function<void(int, const String&)> callback) {
        OSXMessageBox::show(AlertWindow::AlertIconType::NoIcon, title, message, {},
                                   "Yes", "No", nullptr, callback != nullptr);
    }
...
//==============================================================================
    ...
    struct NSDraggingSourceHelper : public ObjCClass<NSObject <NSDraggingSource>> {
        NSDraggingSourceHelper() : ObjCClass<NSObject <NSDraggingSource>>("JUCENSDraggingSourceHelper_") {
            ...
            addMethod(@selector(draggingSession:sourceOperationMaskForDraggingContext:),
                      sourceOperationMaskForDraggingContext, "c@:@@");
    ...
    private:
        ...
        sourceOperationMaskForDraggingContext(id self, SEL, NSDraggingSession *, NSDraggingContext) {
        ...
        static void draggingSessionEnded(id self, SEL, NSDraggingSession *, NSPoint p, NSDragOperation) {
            ...
            if (auto *view = getNSViewForDragEvent(nullptr))
                if (auto *cgEvent = CGEventCreateMouseEvent(nullptr, kCGEventLeftMouseUp, CGPointMake(p.x, p.y),
                                                            kCGMouseButtonLeft))
            ...
            JUCE_AUTORELEASEPOOL {
                if (auto event = [[view window] currentEvent]) {
                    ...
                    for (auto &filename : files) {
                        ...
                        auto eventPos = [event locationInWindow];
                        auto dragRect = [view convertRect:NSMakeRect(eventPos.x - 16.0f, eventPos.y - 16.0f, 32.0f,
                                                                     32.0f)
                                                 fromView:nil];
